...
util.success = function(commandName) {
  ...
  if (approvedAnalytics) {
    sendAnalyticsEvent(commandName);
  }
};
...
util.error = function(commandName, msg) {
  ...
  if (approvedAnalytics) {
    sendAnalyticsEvent(commandName, true);
  }
};
...
function sendAnalyticsEvent (command, failed) {
  const uid = confUtils.getConf('uid');
  const visitor = ua('UA-106613728-5');
  const params = {
    ec: 'CLI',
    ea: failed ? 'error' : 'exec',
    el: `${command} ${cliVersion}`,
    uid,
    aip: true
  };
