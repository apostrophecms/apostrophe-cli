...
module.exports = function (program) {
  ...
  program
    .command('createWidget <widgetName>')
    ...
    .action(function(widgetName, options) {
      ...
      util.nlog('createWidget', 'Adding '+ widgetName +'-widgets folder to /lib/modules.');
      ...
      util.nlog('createWidget', 'Creating a views folder and widget.html for ' + fullWidgetName + '.');
      mkdir('-p', path + '/views');
      ...
      util.nlog('createWidget', 'Setting up index.js for '+ fullWidgetName + '.');
      fs.writeFileSync(path + '/index.js', widgetConfig);

      if(options.player) {
        util.nlog('createWidget', 'Setting up always.js for '+ widgetName + fullWidgetName +'.');
        mkdir('-p', path + '/public/js');

        var jsConfig = "apos.define('"+ fullWidgetName +"', {\
          \n  extend: 'apostrophe-widgets',\
